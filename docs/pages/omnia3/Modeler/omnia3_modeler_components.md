...
| exportCSV | Method that requests to the API a data export to CSV. Has the following optional parameters:<br><br>
<ul><li>the name of the file to download</li><li>a boolean that indicates if the result modal auto dismisses (default: true)</li><li>the time (in ms) to dismiss the result modal (default: 5000ms)</li><li>the time (in ms) to dismiss the informative modal (default: 5000ms)</li></ul> |
